...
** Installation
Example configuration with =use-package=. Note, that for Ejira to work =org-id-track-globally= needs to be set to =t=, as =org-id=-lookups are heavily used by Ejira. Pushes are reviewed in an [[https://github.com/vleonbonnet/org-sync-confirm][org-sync-confirm]] buffer, which must be installed alongside.
#+BEGIN_SRC elisp
  (use-package ejira
    :init
    (setq jiralib2-url              "https://jira.example.com"
          jiralib2-auth             'basic
...
** Jira content projection

An ejira heading normally uses its Org heading as the Jira summary and its
=Description= child as the Jira description.  When a task needs detailed local
notes but a compact external Jira presentation, add =JIRA_TITLE= and, if needed,
a direct =JIRA_DESCRIPTION= child:

#+BEGIN_SRC org
  ,*** STARTED Implement callback recovery
  :PROPERTIES:
  :JIRA_TITLE: Complete callback recovery
  :END:

  Detailed local design notes stay here and are not sent to Jira.

  ,**** JIRA_DESCRIPTION
  Concise Jira-facing progress description.
#+END_SRC

The property and child form a bidirectional Jira projection.  Ejira pushes only
the projected title and description, pulls Jira edits back into those fields,
and leaves the local heading, body, and ordinary =Description= subtree intact.
With =JIRA_TITLE= but no =JIRA_DESCRIPTION= child, the Jira description is
deliberately empty.
...
| =C-c= ,                    | =ejira-set-priority=           | Set the priority from Jira's issue-specific scheme and sync its exact ID.                            |
...
Priority cookies are ordered ranks from Jira's issue-specific edit metadata.
Ejira stores the exact priority ID in =JiraPriorityId= and sends that ID back
to Jira.  Use =ejira-priority-policies= when Jira exposes backend priorities
that should not appear in the project's user-facing priority menu.

The end-to-end test requires external configuration.  Set =EJIRA_E2E_URL=,
=EJIRA_E2E_PROJECT=, and =EJIRA_E2E_TOKEN= in the environment; no server URL,
project key, or credential-store entry is included in the source.
...
*** Push review
Saving a managed org buffer, or =ejira-push-item-under-point=, opens the =*ejira-confirm*= buffer, an [[https://github.com/vleonbonnet/org-sync-confirm][org-sync-confirm]] review of every pending Jira write: summary and description edits, comment edits, new issues, subtasks and comments, comment deletions, state transitions, issue type and epic changes.
...
- Each write is an item with a =[x]= checkbox; =RET= unticks it and the heading stays dirty for the next save.
- Fields expand with =TAB= to a word-refined diff; =payload= shows the Jira markup that will be sent.
- =Assign new issues to me= and the per-issue =assign to me= boxes control assignment of created issues.
- =C-c C-c= pushes the ticked items; failures are collected into one warning and the other items still go through. =C-c C-k= cancels.
